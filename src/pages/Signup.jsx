...
import { signupUser, loginUser } from "../api/auth";   // ✅ FIX: Import API
...
export const Signup = () => {
  ...
  async function handleSubmit(e) {
    ...
    try {
      const signupRes = await signupUser(form);   // ✅ Use API

      // Auto-login after successful signup to obtain token and user
      try {
        const loginRes = await loginUser({ email: form.email, password: form.password });
        if (loginRes.token) {
          localStorage.setItem("token", loginRes.token);
          localStorage.setItem("user", JSON.stringify(loginRes.user || signupRes.user));
        }
      } catch (loginErr) {
        console.warn("Auto-login failed after signup:", loginErr);
      }

      setMessage({ text: "Signup successful! Redirecting to homepage...", type: "success" });
