...
//         }
//       } catch (err) {
//         // Silent fail for wishlist fetch errors to improve UX if backend is down
//         console.warn("Could not fetch wishlist (backend might be down):", err);
//       }
//     };
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="relative flex h-16 md:h-20 items-center">
            ...
            <div className="hidden lg:flex items-center gap-4 ml-auto z-10">
              ...
              <div className="relative">
                {user ? (
                  ...
                  // When NOT logged in - only show Sign up button, no "Log in" text
                  <button
            ...
            <button
              onClick={() => setMenuOpen(!menuOpen)}
              className={`lg:hidden p-2 rounded-lg transition-all ml-auto ${scrolled ? 'hover:bg-gray-100' : 'hover:bg-white/10'}`}
              aria-label="Toggle menu"
            >
              {menuOpen ? (
                <X className={`w-6 h-6 ${textColor}`} />
