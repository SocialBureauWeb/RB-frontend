...
export default function PropertyCard({ property, onWishlistChange }) {
  ...
  async function toggleSaved(e) {
    ...
    if (!token) {
      toast.error("Please login to save properties", {
        duration: 800, // Shorter duration for immediate redirect
        icon: '🔒',
      });
      ...
      }, 500); // Reduced to 500ms for quicker redirect
      return;
    }
      ...
      if (data.success) {
        ...
        toast.success(
          saved
            ? "Property removed from wishlist"
            : "Property added to wishlist",
          { duration: 2000 } // Normal duration for success messages
        );
      } else {
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-[4/3] bg-gray-100">
        ...
        <button
          onClick={toggleSaved}
          className="absolute top-3 right-3 z-20 p-1 rounded-full bg-white bg-opacity-80 hover:bg-opacity-100 transition-all shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
          title={saved ? "Remove from wishlist" : "Add to wishlist"}
          disabled={loading}
        >
          {loading ? (
